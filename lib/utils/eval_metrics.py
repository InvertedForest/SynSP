...
def evaluate_smoothnet_3D(model, test_dataloader, device, dataset_name,estimator,cfg,flg):
    ...
    for i, data in tqdm(enumerate(test_dataloader), total=test_dataloader.dataset.__len__()):
        ...
        if len(denoised_pos.shape)==4: # multi-view
            person = denoised_pos.shape[1]
            a_denoised_pos = denoised_pos
            a_data_pred = data_pred
            a_data_gt = data_gt
        else:
            person = 1
            a_denoised_pos = denoised_pos.unsqueeze(1)
            a_data_pred = data_pred.unsqueeze(1)
            a_data_gt = data_gt.unsqueeze(1)
        for i in range(person):
            denoised_pos = a_denoised_pos[:,i]
            data_pred = a_data_pred[:,i]
            data_gt = a_data_gt[:,i]
            denoised_pos = slide_window_to_sequence(denoised_pos,cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,cfg.MODEL.SLIDE_WINDOW_SIZE)
            # print(time.time()-start_time)
            data_pred = slide_window_to_sequence(data_pred,cfg.EVALUATE.SLIDE_WINDOW_STEP_SIZE,cfg.MODEL.SLIDE_WINDOW_SIZE)
...
def evaluate_smoothnet_smpl(model, test_dataloader, device,cfg,dataset, flg):
    ...
    for i, data in tqdm(enumerate(test_dataloader), total=test_dataloader.dataset.__len__()):
        ...
        with torch.no_grad():
            ...
            start_time=time.time()
            denoised_pos = model(data_pred_pose)[flg]
            inference_time["smoothnet"]+=time.time()-start_time
            # data_pred_pose=data_pred_pose.permute(0,2,1)
            # denoised_pos=denoised_pos.permute(0,2,1)
